...
**Board model**: `ROWS×COLS` 2D array, 0 = empty, 1–7 = piece color index.

**Pieces**: Defined as 2D matrices in `PIECES[1..7]`. Rotation via `rotateCW()` (transpose + row reverse). Collision detection via `collide(shape, ox, oy)`. Wall kicks try offsets [0, -1, 1, -2, 2] in `tryRotate()`.
